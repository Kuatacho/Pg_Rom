//CODIGO ANTIGUO!!! NO BORRAR POR SI ACASO
...
export class HandPrediction implements OnInit, OnDestroy {
  ...
  private sequence: number[][] = [];
  private readonly SEQUENCE_LENGTH = 30;        // número de frames
  private readonly MIN_MOVEMENT = 0.02;         // umbral de movimiento
  private sending = false;
  private firstActiveFrameTime = 0;              // timestamp del primer frame válido
  private readonly MIN_GESTURE_MS = 3000;       // 4 segundos mínimo
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(pm => {
      ...
      this.gesturesService.getGestosPorLeccion(this.leccionId).subscribe({
        next: gestos => {
          this.totalGestos = gestos.length;
          this.isLast = this.gestoId >= this.totalGestos;
          this.gestoEsperado = gestos.find(g => Number(g.id) === this.gestoId) || null;
          this.cdr.detectChanges();
        }
      });
    ...
    try {
      ...
      this.cameraInstance = new Camera(video, {
        onFrame: async () => {
          ...
          this.sizeCanvasToVideo();
          try { await this.handsInstance.send({ image: video }); } catch {}
        },
        width: video.videoWidth || 640,
        height: video.videoHeight || 480,
      });
  ...
  private onResults(results: Results) {
    ...
    if (hasHands) {
      this.sequence.push(flat);
      if (!this.firstActiveFrameTime) this.firstActiveFrameTime = Date.now();
    }
    ...
    // ===== Condición de envío =====
    const elapsed = Date.now() - this.firstActiveFrameTime;
    const ready = hasHands &&
      this.sequence.length === this.SEQUENCE_LENGTH &&
      movement >= this.MIN_MOVEMENT &&
      elapsed >= this.MIN_GESTURE_MS &&
      !this.sending;

    if (ready) {
      this.sendToBackend(this.sequence.slice(-this.SEQUENCE_LENGTH));
      this.firstActiveFrameTime = 0; // reset para próximo gesto
    }
